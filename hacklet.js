javascript: (async function() {
    const inQuestionText = document.querySelector('[class*="question-title__Title-"]');
    const inPreQuestionText = document.querySelector('[class*="block-title__Title-"]');
    let inQuestion = inQuestionText !== null;
    let text = inQuestion ? inQuestionText.textContent : inPreQuestionText.textContent;
    let answers = [];
    let answerHtml = document.querySelectorAll('[data-functional-selector*="question-choice-text-"]');
    ...
    if (inQuestion) {
        ...
        const targetButton = document.querySelector('[data-functional-selector="answer-' + answer + '"]');
        if (targetButton) {
            targetButton.setAttribute("style", targetButton.getAttribute("style") + "border: 5px solid white; ");
        } else {
